...
class Coco:
    ...
    def __init__(self, img_dir=None, dtype='train'):
        ...
        #     self.anno = json.load(anno_file)
        with open('data/coco_data/val_dets.json') as anno_file:
             self.anno = json.load(anno_file)
    ...
    def __getitem__(self, ind):
        ...
        im = self.coco.loadImgs(self.anno[ind]['imgInfo']['imgID'])[0]['file_name']
        path = self.img_dir + im
        images, imagesf, warps = apply_augmentation_test_td(path, self.anno[ind], output_size=256)
        meta = {'index': ind, 'imgID': self.anno[ind]['imgInfo']['imgID'],
